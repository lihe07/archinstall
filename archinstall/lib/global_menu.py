...
class GlobalMenu(AbstractMenu):
    ...
    def setup_selection_menu_options(self):
        ...
        self._menu_options["locale_config"] = Selector(
            _("Locales"),
        ...
        self._menu_options["mirror_config"] = Selector(
            _("Mirrors"),
        ...
        self._menu_options["disk_encryption"] = Selector(
            _("Disk encryption"),
        ...
        self._menu_options["swap"] = Selector(
            _("Swap"), lambda preset: ask_for_swap(preset), default="zram"
        )
        ...
        self._menu_options["uki"] = Selector(
            _("Unified kernel images"),
            lambda preset: ask_for_uki(preset),
            default=False,
        )
        self._menu_options["hostname"] = Selector(
            _("Hostname"), lambda preset: ask_hostname(preset), default="archlinux"
        )
        ...
        self._menu_options["network_config"] = Selector(
        ...
        self._menu_options["timezone"] = Selector(
            _("Timezone"), lambda preset: ask_for_a_timezone(preset), default="UTC"
        )
        self._menu_options["ntp"] = Selector(
            _("Automatic time sync (NTP)"), lambda preset: ask_ntp(preset), default=True
        )
        ...
        self._menu_options["abort"] = Selector(
            _("Abort"), exec_func=lambda n, v: exit(1)
        )
    ...
    def _missing_configs(self) -> List[str]:
        ...
        mandatory_fields = dict(
            filter(lambda x: x[1].is_mandatory(), self._menu_options.items())
        )
        missing = set()
    ...
    def _disk_encryption(
        self, preset: Optional[disk.DiskEncryption]
    ) -> Optional[disk.DiskEncryption]:
        disk_config: Optional[disk.DiskLayoutConfiguration] = self._menu_options[
            "disk_config"
        ].current_selection
        ...
        disk_encryption = disk.DiskEncryptionMenu(
            disk_config, data_store, preset=preset
        ).run()
    ...
    def _prev_network_config(self) -> Optional[str]:
        selector: Optional[NetworkConfiguration] = self._menu_options[
            "network_config"
        ].current_selection
        if selector:
    ...
    def _prev_disk_config(self) -> Optional[str]:
        ...
        disk_layout_conf: Optional[disk.DiskLayoutConfiguration] = (
            selector.current_selection
        )

        output = ""
        if disk_layout_conf:
            output += str(_("Configuration type: {}")).format(
                disk_layout_conf.config_type.display_msg()
            )

            if disk_layout_conf.lvm_config:
                output += "\n{}: {}".format(
                    str(_("LVM configuration type")),
                    disk_layout_conf.lvm_config.config_type.display_msg(),
                )
    ...
    def _display_disk_config(
        self, current_value: Optional[disk.DiskLayoutConfiguration] = None
    ) -> str:
        if current_value:
    ...
    def _prev_disk_encryption(self) -> Optional[str]:
        ...
        encryption: Optional[disk.DiskEncryption] = self._menu_options[
            "disk_encryption"
        ].current_selection
        ...
        if encryption:
            ...
            output += (
                str(_("Password")) + f": {secret(encryption.encryption_password)}\n"
            )

            if encryption.partitions:
                output += (
                    "Partitions: {} selected".format(len(encryption.partitions)) + "\n"
                )
            elif encryption.lvm_volumes:
                output += (
                    "LVM volumes: {} selected".format(len(encryption.lvm_volumes))
                    + "\n"
                )
    ...
    def _display_disk_encryption(
        self, current_value: Optional[disk.DiskEncryption]
    ) -> str:
        if current_value:
    ...
    def _validate_bootloader(self) -> Optional[str]:
        ...
        if bootloader == Bootloader.Limine:
            if boot_partition.fs_type != disk.FilesystemType.Fat32:
                return (
                    "Limine does not support booting from filesystems other than FAT32"
                )
    ...
    def _prev_profile(self) -> Optional[str]:
        ...
        if profile_config and profile_config.profile:
            ...
            if profile_config.gfx_driver:
                output += (
                    str(_("Graphics driver"))
                    + ": "
                    + profile_config.gfx_driver.value
                    + "\n"
                )
    ...
    def _select_disk_config(
        self, preset: Optional[disk.DiskLayoutConfiguration] = None
    ...
    def _select_audio(
        self, current: Optional[AudioConfiguration] = None
    ...
    def _mirror_configuration(
        self, preset: Optional[MirrorConfiguration] = None
    ) -> Optional[MirrorConfiguration]:
        data_store: Dict[str, Any] = {}
    ...
    def _prev_mirror_config(self) -> Optional[str]:
        ...
        if selector.has_selection():
            ...
            if mirror_config.regions:
                output += "{}: {}\n\n".format(
                    str(_("Mirror regions")), mirror_config.regions
                )

...
def select_kernel(preset: List[str] = []) -> List[str]:
    ...
    choice = Menu(
        _('Choose which kernels to use or leave blank for default "{}"').format(
            default_kernel
        ),
        kernels,
        sort=True,
...
def ask_for_bootloader(preset: Bootloader) -> Bootloader:
    ...
    match choice.type_:
        case MenuSelectionType.Skip:
            return preset
        case MenuSelectionType.Selection:
            return Bootloader(choice.value)
...
def ask_for_uki(preset: bool = True) -> bool:
    ...
    choice = Menu(
        prompt, Menu.yes_no(), default_option=Menu.no(), preset_values=preset_val
    ).run()
...
def select_driver(
    options: List[GfxDriver] = [], current_value: Optional[GfxDriver] = None
) -> Optional[GfxDriver]:
    """
...
def ask_for_swap(preset: str = "zram") -> str:
    ...
    prompt = "Which swap method would you like to use?"

    options = ["zram", "swapfile", "none"]

    choice = Menu(
        prompt, options, default_option="zram", sort=False, preset_values=preset_val
    ).run()
